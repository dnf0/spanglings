...
use spanglings::cli::commands::progress::{compute_weakness_profile, get_progress_json, ProgressSummary};
...
fn test_weakness_profiler_detects_low_ease_and_lapses() {
    ...
    state.srs.insert("b1_subj_weirdo_wishes".to_string(), subj_item);
    state.stats.insert(
    ...
    assert!(!profile.is_empty(), "Weakness profile should detect weak topics");
    ...
    assert!(top.recommendation.contains("spanglings explain") || top.recommendation.contains("spanglings drill"));
}
