...
```mermaid
graph TD
    A[present_indicative_regular] --> B[present_indicative_irregular]
    A --> C[ser_vs_estar_basic]
    B --> D[present_subjunctive_regular]
    B --> E[irregular_preterite_stems]
    E --> F[imperfect_subjunctive_forms]
    D --> G[subjunctive_wishes_desires]
    D --> H[subjunctive_doubt_denial]
