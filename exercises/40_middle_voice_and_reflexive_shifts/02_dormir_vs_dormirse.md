# Middle-Voice & Reflexive Shifts 02: Dormir vs Dormirse (State vs Inchoative Transition)
<!-- id: b1_reflex_dormir_vs_dormirse | level: B1 | topic: middle_voice_and_reflexive_shifts | type: cloze | concepts: ["middle_voice_reflexive_shifts"] | prerequisites: ["passive_refleja_vs_impersonal", "accidental_se_structures"] | grammar_focus: "Middle voice, inchoative aspect, and telic reflexive shifts." | contrast_note: "Contrast non-reflexive base verb with reflexive aspectual/inchoative nuance." -->

> **Grammar & Lexicon**: **Dormir** denotes the continuous state of sleep (*dormí ocho horas*). **Dormirse** is the inchoative transition: the exact moment of falling asleep (*me dormí a medianoche*, *se durmió en el autobús*).

### Context
English: "He was so exhausted that he fell asleep in the middle of the lecture without even realizing it."

### Exercise
Estaba tan exhausto que (se durmió en plena conferencia sin / se durmio en plena conferencia) ___ siquiera darse cuenta.

<!-- SOLUTION
se durmió en plena conferencia sin
-->

<!-- ALTERNATIVES
se durmio en plena conferencia sin
se durmió a mitad de la conferencia sin
se durmio a mitad de la conferencia sin
se quedó dormido en plena conferencia sin
-->

<!-- DIAGNOSTIC_RULES
pattern: "durmió en plena conferencia" | code: "E4002" | message: "The act of 'falling asleep' requires the reflexive/pronominal form 'se durmió' (or 'se quedó dormido')."
-->

<!-- HINTS
Tier 1: Use preterite of 'dormirse' (3rd person singular with stem change: se durmió) + 'en plena conferencia sin'.
Tier 2: 'se' + 'durmió' + 'en' + 'plena' + 'conferencia' + 'sin'.
Tier 3: Solution: 'se durmió en plena conferencia sin'.
-->
