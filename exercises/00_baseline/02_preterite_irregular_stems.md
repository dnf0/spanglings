<!-- I AM NOT DONE -->
# Baseline 02: Irregular Preterite Stems
<!-- id: b0_preterite_stems_01 | level: Baseline | topic: baseline_preterite_stems | type: cloze -->

> **Grammar Rule**: Verbs with irregular preterite stems (hacer -> hic-/hiz-, poner -> pus-, tener -> tuv-) take unaccented endings -e, -iste, -o, -imos, -isteis, -ieron.

### Context
English: "Yesterday I did all the housework."

### Exercise
Ayer yo (hacer) ___ todas las tareas de la casa.

<!-- SOLUTION
hice
-->

<!-- ALTERNATIVES
hice yo
-->

<!-- DIAGNOSTIC_RULES
pattern: "hizo" | code: "E0201" | message: "'hizo' is 3rd person singular (él/ella). For 'yo', use 'hice'."
pattern: "hací" | code: "E0201" | message: "'hacer' has an irregular preterite stem: 'hic-' -> 'hice'."
-->

<!-- HINTS
Tier 1: Irregular preterite stem for 'hacer'.
Tier 2: Root is 'hic-' with 1st person ending '-e'.
Tier 3: The answer is 'hice'.
-->
