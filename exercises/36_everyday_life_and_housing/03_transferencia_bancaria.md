# Everyday Life & Housing 03: Banking Transactions & Wire Fees
<!-- id: b1_housing_transferencia_bancaria | level: B1 | topic: everyday_life_and_housing | type: cloze | concepts: ["lease_agreements_contracts"] | prerequisites: ["por_vs_para_nuances", "passive_refleja_vs_impersonal"] | grammar_focus: "Tenancy contracts, housing logistics, property maintenance, and rental agreements." -->

> **Grammar & Lexicon**: In banking, sending money between institutions is a **transferencia interbancaria** or **transferencia bancaria**, and any transaction charge is a **comisión de transferencia** (or *comisión bancaria*).

### Context
English: "The bank charged a ten euro transaction fee for making an international bank transfer."

### Exercise
El banco cobró una (comisión por realizar una transferencia bancaria / comision por realizar una transferencia bancaria) ___ al extranjero.

<!-- SOLUTION
comisión por realizar una transferencia bancaria
-->

<!-- ALTERNATIVES
comision por realizar una transferencia bancaria
comisión por hacer una transferencia bancaria
comision por hacer una transferencia bancaria
tarifa por hacer una transferencia bancaria
-->

<!-- DIAGNOSTIC_RULES
pattern: "fee de banco" | code: "E3603" | message: "Use the authentic Spanish banking term 'comisión' or 'tarifa', never the English 'fee'."
-->

<!-- HINTS
Tier 1: Translate 'transaction fee for making a bank transfer'.
Tier 2: 'comisión' + 'por' + 'realizar' + 'una' + 'transferencia' + 'bancaria'.
Tier 3: Solution: 'comisión por realizar una transferencia bancaria'.
-->
