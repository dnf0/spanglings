# Consumer Rights 01: Official Complaint Sheet (Hoja de Reclamaciones)
<!-- id: b2_consumer_hoja_reclamaciones | level: B2 | topic: consumer_complaints_and_rights | type: cloze | concepts: ["customer_service_disputes"] | prerequisites: ["subjunctive_emotion_evaluation", "discourse_connectors"] | grammar_focus: "Official complaint protocols in physical establishments." | contrast_note: "Requesting official state-regulated complaint forms vs informal comment cards." -->

> **Grammar Rule**: Every commercial establishment in Spain is legally obligated to provide the *hoja oficial de quejas y reclamaciones* upon request.

### Context
English: "Given your refusal to resolve the issue, I request the official complaint form immediately."

### Instructions
**TODO**: Select the formal consumer complaint request using the courteous 3rd-person indirect object clitic with the 1st-person singular present indicative *(solicitar)* and the official regulatory title.
**Why**: Exercising statutory consumer rights requires formal administrative register (*solicitar con pronombre de cortesía*) and the complete legal title of the complaint document.

### Exercise
<!-- TODO: Select the polite formal 1st-person request for the official consumer complaint form -->
Ante su negativa a resolver la incidencia, (le solicito la hoja oficial de quejas y reclamaciones / exijo la hoja de reclamaciones) ___ para remitirla a la Oficina Municipal de Información al Consumidor.
<!-- SOLUTION
le solicito la hoja oficial de quejas y reclamaciones
-->

<!-- ALTERNATIVES
exijo la hoja de reclamaciones
solicito la hoja de reclamaciones
-->

<!-- DIAGNOSTIC_RULES
pattern: "deme el libro de quejas" | code: "E0044" | message: "In Spain, the legal statutory term is 'la hoja oficial de quejas y reclamaciones'."
-->

<!-- HINTS
Tier 1: 'le solicito la hoja oficial de quejas y reclamaciones'.
Tier 2: Include 'hoja oficial de quejas y reclamaciones'.
Tier 3: Enter 'le solicito la hoja oficial de quejas y reclamaciones'.
-->
