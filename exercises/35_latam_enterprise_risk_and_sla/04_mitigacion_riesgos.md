# LatAm Enterprise SLA 04: Risk Mitigation & Contingency Planning
<!-- id: c1_sla_mitigacion_riesgos | level: C1 | topic: latam_enterprise_risk_and_sla | type: cloze | concepts: ["tech_software_engineering"] | prerequisites: ["executive_leadership_communication"] | grammar_focus: "Enterprise risk management, SLA negotiations, and compliance dialogue." -->

> **Grammar & Lexicon**: Enterprise procurement security reviews require submitting a formal **plan de mitigación de riesgos** and a business continuity plan (*plan de continuidad del negocio*).

### Context
English: "The chief security officer presented a comprehensive risk mitigation plan to comply with regional financial regulations."

### Instructions
**TODO**: Insert the executive governance noun phrase for risk abatement strategy with proper tilde on the action noun.
**Why**: Enterprise security governance pairs the strategy blueprint noun with the prepositional hazard abatement phrase.

### Exercise
<!-- TODO: Supply the executive governance phrase for risk abatement strategy -->
El director de seguridad de la información presentó un exhaustivo (plan de mitigación de riesgos / plan de mitigacion de riesgos) ___ para cumplir con la normativa bancaria regional.

<!-- SOLUTION
plan de mitigación de riesgos
-->

<!-- ALTERNATIVES
plan de mitigacion de riesgos
programa de mitigación de riesgos
programa de mitigacion de riesgos
-->

<!-- DIAGNOSTIC_RULES
pattern: "plan para mitigar el riesgo" | code: "E3504" | message: "The standard institutional noun phrase is 'plan de mitigación de riesgos'."
-->

<!-- HINTS
Tier 1: Translate 'risk mitigation plan'.
Tier 2: 'plan' + 'de' + 'mitigación' + 'de' + 'riesgos'.
Tier 3: Solution: 'plan de mitigación de riesgos'.
-->
