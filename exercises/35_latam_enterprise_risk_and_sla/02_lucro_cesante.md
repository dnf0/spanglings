# LatAm Enterprise SLA 02: Consequential Damages & Lost Profits
<!-- id: c1_sla_lucro_cesante | level: C1 | topic: latam_enterprise_risk_and_sla | type: cloze | concepts: ["tech_software_engineering"] | prerequisites: ["executive_leadership_communication"] | grammar_focus: "Enterprise risk management, SLA negotiations, and compliance dialogue." -->

> **Grammar & Lexicon**: In civil law jurisdictions throughout Latin America, liability limitations distinguish between direct damages (*daño emergente*) and lost profits / consequential damages (**lucro cesante**). SaaS contracts routinely exclude liability for *lucro cesante*.

### Context
English: "The service agreement explicitly excludes any liability for lost profits or indirect commercial damages."

### Exercise
El contrato de prestación de servicios excluye expresamente cualquier responsabilidad por (lucro cesante o daños indirectos / lucro cesante o danos indirectos) ___ derivados de interrupciones imprevistas.

<!-- SOLUTION
lucro cesante o daños indirectos
-->

<!-- ALTERNATIVES
lucro cesante o danos indirectos
lucro cesante o perjuicios indirectos
-->

<!-- DIAGNOSTIC_RULES
pattern: "ganancias perdidas" | code: "E3502" | message: "In Latin American commercial contract law, the technical term for lost profits is 'lucro cesante', not literal 'ganancias perdidas'."
-->

<!-- HINTS
Tier 1: Use the Latin American legal term for lost profits ('lucro cesante') + 'o daños indirectos'.
Tier 2: 'lucro' + 'cesante' + 'o' + 'daños' + 'indirectos'.
Tier 3: Solution: 'lucro cesante o daños indirectos'.
-->
