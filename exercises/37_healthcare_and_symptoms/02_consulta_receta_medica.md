# Healthcare & Symptoms 02: Doctor Consultations & Prescriptions
<!-- id: b1_health_consulta_receta | level: B1 | topic: healthcare_and_symptoms | type: cloze | concepts: ["accidental_se_structures"] | prerequisites: ["gustar_dative_structures", "por_vs_para_nuances"] | grammar_focus: "Medical consultations, describing symptoms, physical sensations, and treatments." -->

> **Grammar & Lexicon**: When a physician prescribes medication, they **recetan un fármaco / antibiótico**. The dosage instructions are given with the distributive formula: **tomar una dosis / pastilla cada ocho horas**.

### Context
English: "The physician decided to prescribe an antibiotic and instructed him to take one tablet every eight hours with food."

### Instructions
**TODO**: Fill in the authentic Spanish phrase or clinical/contractual term matching the context.
**Why**: When a physician prescribes medication, they **recetan un fármaco / the required term**. The dosage instructions are given with the distributive formula: **tomar una the required term / pastilla the required term the required term**.

### Exercise
<!-- TODO: Fill in the authentic Spanish phrase or clinical/contractual term matching the context. -->
El médico decidió (recetarle un antibiótico e indicarle que tomara una dosis cada ocho horas / recetarle un antibiotico) ___ junto con las comidas principales.

<!-- SOLUTION
recetarle un antibiótico e indicarle que tomara una dosis cada ocho horas
-->

<!-- ALTERNATIVES
recetarle un antibiotico e indicarle que tomara una dosis cada ocho horas
recetarle un antibiótico e indicarle tomar una dosis cada ocho horas
recetarle un antibiótico e indicarle que tome una dosis cada ocho horas
recetarle un antibiotico e indicarle que tome una dosis cada ocho horas
-->

<!-- DIAGNOSTIC_RULES
pattern: "prescribirle una medicina" | code: "E3702" | message: "While 'prescribir' is valid, the most natural everyday term in medical care is 'recetarle un antibiótico'."
-->

<!-- HINTS
Tier 1: Translate 'prescribe him an antibiotic and instruct him to take one dose every eight hours'.
Tier 2: 'recetarle' + 'un' + 'antibiótico' + 'e' + 'indicarle' + 'que' + 'tomara' + 'una' + 'dosis' + 'cada' + 'ocho' + 'horas'.
Tier 3: Solution: 'recetarle un antibiótico e indicarle que tomara una dosis cada ocho horas'.
-->
