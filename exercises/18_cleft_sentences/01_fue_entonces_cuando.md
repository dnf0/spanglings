<!-- I AM NOT DONE -->
# Cleft Sentences 01: Temporal Focalization ('Fue entonces cuando')
<!-- id: c1_cleft_fue_entonces | level: C1 | topic: cleft_sentences | type: cloze -->

> **Grammar Rule**: In Spanish temporal clefts, use 'ser + adverbio de tiempo + cuando' (e.g. 'Fue entonces cuando...', not 'fue entonces que...').

### Context
English: "It was then when I truly understood the dilemma."

### Exercise
Fue entonces ___ comprendí la gravedad del dilema.

<!-- SOLUTION
cuando
-->

<!-- ALTERNATIVES
cuando
-->

<!-- DIAGNOSTIC_RULES
pattern: "que" | code: "E0001" | message: "In standard Spanish temporal clefts, match temporal adverb with 'cuando' ('Fue entonces cuando'), avoiding queísmo."
-->

<!-- HINTS
Tier 1: Relative adverb matching temporal antecedent.
Tier 2: 'cuando'.
Tier 3: The answer is 'cuando'.
-->
