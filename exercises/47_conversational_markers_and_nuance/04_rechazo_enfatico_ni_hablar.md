# Conversational Markers 04: Emphatic Refusal: Ni hablar & Ni de broma
<!-- id: b2_marker_ni_hablar | level: B2 | topic: conversational_markers_and_nuance | type: cloze | concepts: ["idiomatic_expressions_colloquial"] | prerequisites: ["discourse_connectors", "register_elevation_formal"] | grammar_focus: "Categorical refusal and emphatic negative markers." | contrast_note: "Absolute rejection formulas without subjunctive embedding." -->

> **Grammar Rule**: Categorical refusal expressions (*¡Ni hablar!, ¡Ni de coña!, ¡Ni por asomo!*) reject outright unacceptable proposals.

### Context
English: "Out of the question! I won't accept an unfair contract that removes all our intellectual property rights."

### Instructions
**TODO**: Insert the authentic emphatic conversational idiom of categorical rejection.
**Why**: Categorical refusal expressions reject outright unacceptable proposals with assertive exclamation marks and negative polarity.

### Exercise
<!-- TODO: Insert the authentic emphatic conversational idiom of categorical refusal -->
(¡Ni hablar! No pienso aceptar / ¡Ni de broma voy a firmar) ___ un contrato leonino que suprima todos nuestros derechos de autor.

<!-- SOLUTION
¡Ni hablar! No pienso aceptar
-->

<!-- ALTERNATIVES
¡Ni de broma voy a firmar
¡Ni hablar! No voy a aceptar
Ni hablar! No pienso aceptar
-->

<!-- DIAGNOSTIC_RULES
pattern: "No quiero firmar eso" | code: "E0047" | message: "Practice the emphatic colloquial rejection: '¡Ni hablar! No pienso aceptar'."
-->

<!-- HINTS
Tier 1: '¡Ni hablar! No pienso aceptar'.
Tier 2: Include opening inverted exclamation mark '¡'.
Tier 3: Type '¡Ni hablar! No pienso aceptar'.
-->
